"""Create and locate revisioned dwarfs packs: "<target>-rev<N>.dfs"."""
...
def latest_pack(paths: TargetPaths) -> Path | None:
    revisions = revisions_for_target(paths)
    return revisions[-1][1] if revisions else None


def next_pack_path(paths: TargetPaths) -> Path:
    revisions = revisions_for_target(paths)
...
def create_pack(paths: TargetPaths, source_dir: Path) -> Path:
    ...
    command += [
        ...
        "--categorize",
        "--no-history",
        "--compression=zstd:level=12:wlog=27:strat=3",
        f"--input={source_dir}",
...
def pack_source_dir(source_dir: Path) -> Path:
